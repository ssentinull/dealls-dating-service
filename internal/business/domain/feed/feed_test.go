...
func TestFeedDomain_GetFeedByParams(t *testing.T) {
	...
	getFeedByParamSQL := `WITH potential_matches AS ( SELECT u.id, u.name, u.gender, u.location, u.created_at, DATE_PART('year', AGE(u.birth_date)) AS age FROM users u WHERE DATE_PART('year', AGE(u.birth_date)) <= $1 AND DATE_PART('year', AGE(u.birth_date)) >= $2 AND u.id != $3 AND u.deleted_at IS NULL ), swiped_today AS ( SELECT s.to_user_id FROM swipes s WHERE s.from_user_id = $4 AND s.created_at::DATE = CURRENT_DATE ) SELECT pm.* FROM potential_matches pm LEFT JOIN swiped_today st ON pm.id = st.to_user_id WHERE st.to_user_id IS NULL ORDER BY pm.id DESC OFFSET $5 LIMIT $6;`
	countFeedByParamSQL := `WITH potential_matches AS ( SELECT u.id, u.name, u.gender, u.location, DATE_PART('year', AGE(u.birth_date)) AS age FROM users u WHERE DATE_PART('year', AGE(u.birth_date)) <= $1 AND DATE_PART('year', AGE(u.birth_date)) >= $2 AND u.id != $3 AND u.deleted_at IS NULL ), swiped_today AS ( SELECT s.to_user_id FROM swipes s WHERE s.from_user_id = $4 AND s.created_at::DATE = CURRENT_DATE ) SELECT COUNT(*) FROM potential_matches pm LEFT JOIN swiped_today st ON pm.id = st.to_user_id WHERE st.to_user_id IS NULL;`
